...
if(!event_object)return;this.dispatchEvent(event_object);}else{if(!e.match(re_on))e="on"+e;event_object=document.createEventObject(orig_e);this.fireEvent(e,event_object);}};$.pnotify.defaults={title:false,title_escape:false,text:false,text_escape:false,styling:"bootstrap",addclass:"",cornerclass:"",nonblock:false,nonblock_opacity:.2,history:true,width:"300px",min_height:"16px",type:"notice",icon:true,animation:"fade",animate_speed:"slow",opacity:1,shadow:true,closer:true,closer_hover:true,sticker:true,sticker_hover:true,hide:true,delay:8000,mouse_reset:true,remove:true,insert_brs:true,stack:{"dir1":"down","dir2":"left","push":"bottom","spacing1":25,"spacing2":25}};})(jQuery);(function($){$.fn.userExperience=function(params){var Ux={initAppLayout:function(){if($('body').hasClass('layout')&&!$('body').data('ui-layout-loaded')){$('body').data('ui-layout-loaded',true);this.appLayout=$('body').layout({useStateCookie:true,cookie:{name:"sociable.ux.layout",expires:90},defaults:{applyDefaultStyles:false,fxName:'drop',fxSpeed:300,spacing_closed:50,spacing_open:10,resizerTip:'Ouvrir/fermer',sliderTip:'Redimmensionner',fxSettings_open:{easing:'easeOutBounce'},fxSettings_close:{easing:'easeOutBounce'},enableCursorHotkey:false},center:{applyDefaultStyles:false,spacing_closed:0,spacing_open:0,size:'100%'},north:{applyDefaultStyles:false,showOverflowOnHover:true,spacing_closed:0,spacing_open:0,togglerLength_open:0,size:52,initClosed:false},south:{applyDefaultStyles:false,spacing_closed:0,spacing_open:0,size:90,togglerLength_closed:"100%",togglerLength_open:50,initClosed:true,onopen_end:function(){return false;},onclose_end:function(){return false;}},east:{size:150,applyDefaultStyles:false,initClosed:true,togglerLength_closed:0,togglerLength_open:0,spacing_open:0,spacing_closed:0,onopen_end:function(){},onclose_end:function(){}},west:{size:320,applyDefaultStyles:false,spacing_open:0,spacing_closed:0,togglerLength_closed:0,initClosed:true,slideTrigger_close:'mouseout',slideTrigger_open:'mouseover',onopen_end:function(){},onclose_end:function(){}}});$('body').on('click','.ui-pane-toggle',function(){if(typeof($(this).attr('data-pane'))!=='undefined'){Ux.appLayout.toggle($(this).attr('data-pane'));}});$('body').on('click','.ui-pane-open',function(){if(typeof($(this).attr('data-pane'))!=='undefined'){Ux.appLayout.open($(this).attr('data-pane'));}});$('body').on('click','.ui-pane-close',function(){if(typeof($(this).attr('data-pane'))!=='undefined'){Ux.appLayout.close($(this).attr('data-pane'));}});$('body').on('click','.ui-pane-show',function(){if(typeof($(this).attr('data-pane'))!=='undefined'){Ux.appLayout.show($(this).attr('data-pane'),false);}});if($('.ui-pane-pin').size()!=0){this.appLayout.addPinBtn('.ui-pane-pin',$('.ui-pane-pin').attr('data-pane'));}
$(window).unload(function(){this.appLayout.save();});}},toggle:function($oItem){$oItem.togglrClass('hide');},sendNotification:function(sText,sType,sIcon,bCustom){var sClass='';if(bCustom===true){sClass='ui-notification';}
...
var aFormInputs=$formTarget.serializeArray();oParams=$.extend($formTarget.data(),obj.data());if($(sFormSelector+' div[contenteditable=true]').size()!=0){$(sFormSelector+' .ui-editor').each(function(){aFormInputs.push({'name':$(this).attr('data-name'),'value':$(this).parent().find('div[contenteditable=true]:first').html()});});}
oParams.parameters=JSON.stringify(aFormInputs);$.ajax({type:'POST',url:$formTarget.attr('action'),dataType:'json',data:oParams,beforeSend:function(preload){$domTarget.data('initialContent',$domTarget.html());if(!obj.hasClass('sendNotificationOnCallback')){$domTarget.empty();}},success:function(rep){if(!obj.hasClass('sendNotificationOnCallback')){$domTarget.append(rep.content);}else{switch(rep.status){case 1:Ux.sendNotification(rep.content,'success','glyphicon glyphicon-check');break;case 2:Ux.sendNotification(rep.content,'error','glyphicon glyphicon-warning-sign');break;case 3:Ux.sendNotification(rep.content,'error','glyphicon glyphicon-warning-sign');break;case 4:Ux.sendNotification(rep.content,'info','glyphicon glyphicon-warning-sign');break;}}},error:function(err){$domTarget.append($domTarget.data('initialContent'));},complete:function(){$domTarget.removeData('initialContent');if(obj.hasClass('loadOnCallback')){Ux.loadView($($(oHandler).data('load-selector')));}
...
$(sGridSelector+' .item').each(function(){if((iColumnsIndex+1)>iColumnsCount){iColumnsIndex=0;}
$(sGridSelector+' .ui-grid-column').eq(iColumnsIndex).append($(this));iColumnsIndex++;$(this).data('curCol',iColumnsIndex);});});},initForms:function(){$('form').each(function(){if(!$(this).data('HasChangesListnenerFired')){$(this).data('bHasChange',false);$('body').on('change',$(this).attr('id'),function(){$(this).data('bHasChange',true);});$(this).data('HasChangesListnenerFired',true);}});this.initEditableElements();this.initEditors();this.initCheckbox();this.initDatepicker();},initCheckbox:function(){$('.ui-checkbox').each(function(){if(!$(this).data('ui-checkbox-fired')){$(this).data('ui-checkbox-fired',true);$(this).bootstrapSwitch();}});},initDatepicker:function(){$('.ui-datetimepicker').each(function(){if(!$(this).data('uiDateTimePickerFired')){$('.ui-datetimepicker').datetimepicker();$(this).data('uiDateTimePickerFired',true);}});},initCarousels:function(){$('.ui-carousel').each(function(){if(!$(this).data('carouselFired')){var iAutoplay=(($(this).hasClass('noAutoplay'))?0:5000);$(this).carousel({interval:iAutoplay,duration:500});$(this).data('carouselFired',true);}});},initEventListeners:function(){if(!$('body').data('UxListened')){$('body').on('focus','[placeholder]',function(){Ux.sendNotification($(this).attr('placeholder'),'info','glyphicon glyphicon-info-sign');});$('body').on('blur','[placeholder]',function(){Ux.hideNotifications();});$('body').on('click','.ui-sendxhr',function(){Ux.sendXHR($(this));});$('body').on('change','.ui-sendxhronchange',function(){Ux.sendXHR($(this));});$('body').on('click','.ui-sendform',function(){Ux.sendForm($(this));});$('.ui-reload').on('click',function(){if(typeof($(this).data('sreloadtarget'))!=='undefined'){Ux.loadView($($(this).data('sreloadtarget')));}else{Ux.loadView();}
return false;});$('.ui-loadscroll').scroll(function(){if($(this).scrollTop()===($(this).prop('scrollHeight')-$(this).outerHeight())){if($(this).find('.ui-scroll-loadable')){$('.ui-scroll-loadable').each(function(){Ux.loadScroll($(this));});}}
return false;});$('body').on('click','.ui-confirm',function(){if(!confirm('Etes vous sure?')){return false;}});$('body').on('click','.ui-select-all',function(){var sCheckboxSelector=$(this).data('checkbox-class');var bCheckState=$(this).is(':checked');if(typeof(sCheckboxSelector)!=='undefined'){$('.ui-select.'+sCheckboxSelector).prop('checked',bCheckState);$($(this).data('delete-selector')).removeClass('hide');}});$('body').on('click','.ui-select',function(){if($(this).data('delete-selector')!=='undefined'&&$(this).data('select-parent')!=='undefined'){if($($(this).data('select-parent')+' .ui-select:checked').size()>1){$($(this).data('delete-selector')).removeClass('hide');}else{$($(this).data('delete-selector')).addClass('hide');}}});$('body').on('click','.ui-toggle',function(){if($($(this).data('toggle-selector')).size()!==0){$($(this).data('toggle-selector')).toggleClass('hide');}});$('.modal').on('hide',function(){var sSelector=$(this).attr('id');if($(sSelector+' form').size()!==0){$.each($(sSelector+' form'),function(){if($(this).data('bHasChange')){alert('Warning!');}});}});$('body').on('.ui-toggle','click',function(){Ux.toggle($($(this).data('toggle-selector')));});$('.ui-toggle-popover').popover({container:'body',placement:'auto',html:true,animation:true,content:function(){return $($(this).data('popover')).html();}});$('.ui-tip').tooltip({placement:'auto',delay:0});$('body').on('click','.ui-focus',function(){$($(this).data('focus-selector')).focus();});$('body').on('submit','form#general-search',function(){$('.ui-sendform[data-form=#general-search]').trigger('click');return false;});$('body').data('UxListened',true);}},fireUx:function(){this.initEventListeners();this.initAppLayout();this.initGrids();this.initForms();this.formatTimestamps();this.initCarousels();}};return Ux;};})(jQuery);(function($){$.fn.Charts=function(params){var oCharts={chartColours:['#88bbc8','#ed7a53','#9FC569','#bbdce3','#9a3b1b','#5a8022','#2c7282'],initCharts:function(){$('.ui-chart').each(function(){if(!$(this).data('chartsFired')){var options={grid:{show:true,aboveData:true,color:"#3f3f3f",labelMargin:5,axisMargin:0,borderWidth:0,borderColor:null,minBorderMargin:5,clickable:true,hoverable:true,autoHighlight:true,mouseActiveRadius:20},series:{grow:{active:false,stepMode:"linear",steps:50,stepDelay:true},lines:{show:true,fill:true,lineWidth:4,steps:false},points:{show:true,radius:5,symbol:"circle",fill:true,borderColor:"#fff"}},legend:{position:"ne",margin:[0,-25],noColumns:0,labelBoxBorderColor:null,labelFormatter:function(label,series){return label+'&nbsp;&nbsp;';}},yaxis:{min:0},xaxis:{ticks:11,tickDecimals:0},colors:oCharts.chartColours,shadowSize:1,tooltip:true,tooltipOpts:{content:"%s : %y.0",shifts:{x:-30,y:-50}}};$.plot($(this),[{label:"Visits",data:[[1,3+oCharts.randNum()],[2,6+oCharts.randNum()],[3,9+oCharts.randNum()],[4,12+oCharts.randNum()],[5,15+oCharts.randNum()],[6,18+oCharts.randNum()],[7,21+oCharts.randNum()],[8,15+oCharts.randNum()],[9,18+oCharts.randNum()],[10,21+oCharts.randNum()],[11,24+oCharts.randNum()],[12,27+oCharts.randNum()],[13,30+oCharts.randNum()],[14,33+oCharts.randNum()],[15,24+oCharts.randNum()],[16,27+oCharts.randNum()],[17,30+oCharts.randNum()],[18,33+oCharts.randNum()],[19,36+oCharts.randNum()],[20,39+oCharts.randNum()],[21,42+oCharts.randNum()],[22,45+oCharts.randNum()],[23,36+oCharts.randNum()],[24,39+oCharts.randNum()],[25,42+oCharts.randNum()],[26,45+oCharts.randNum()],[27,38+oCharts.randNum()],[28,51+oCharts.randNum()],[29,55+oCharts.randNum()],[30,60+oCharts.randNum()]],lines:{fillColor:"#f2f7f9"},points:{fillColor:"#88bbc8"}},{label:"Unique Visits",data:[[1,oCharts.randNum()-5],[2,oCharts.randNum()-4],[3,oCharts.randNum()-4],[4,oCharts.randNum()],[5,4+oCharts.randNum()],[6,4+oCharts.randNum()],[7,5+oCharts.randNum()],[8,5+oCharts.randNum()],[9,6+oCharts.randNum()],[10,6+oCharts.randNum()],[11,6+oCharts.randNum()],[12,2+oCharts.randNum()],[13,3+oCharts.randNum()],[14,4+oCharts.randNum()],[15,4+oCharts.randNum()],[16,4+oCharts.randNum()],[17,5+oCharts.randNum()],[18,5+oCharts.randNum()],[19,2+oCharts.randNum()],[20,2+oCharts.randNum()],[21,3+oCharts.randNum()],[22,3+oCharts.randNum()],[23,3+oCharts.randNum()],[24,2+oCharts.randNum()],[25,4+oCharts.randNum()],[26,4+oCharts.randNum()],[27,5+oCharts.randNum()],[28,2+oCharts.randNum()],[29,2+oCharts.randNum()],[30,3+oCharts.randNum()]],lines:{fillColor:"#fff8f2"},points:{fillColor:"#ed7a53"}}],options);$(this).data('chartsFired',true);}});},randNum:function(){return(Math.floor(Math.random()*(1+40-20)))+20;}};return oCharts;};})(jQuery);$(document).ready(function(){var Ux=$.fn.userExperience();Ux.loadView();Ux.fireUx();$(document).ajaxStop(function(){Ux.fireUx();});});